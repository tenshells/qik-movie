...
def main():
    ...
    duration_per_media = prompt_with_default("Duration per image/video (seconds)", DURATION_PER_IMAGE, float)
    ...
    all_resized = {**{img: rimg for img, rimg in zip(images, resized_images)}, **{vid: rvid for vid, rvid in zip(videos, resized_videos)}}
    mixed_media = [all_resized[f] for f in sorted(all_files, key=lambda x: os.listdir(img_folder).index(os.path.basename(x))) if f in all_resized]
