...
class MediaResizer:
    ...
    def ensure_resized_folder(self, input_folder: str) -> str:
        ...
        self.resized_folder = os.path.join(
            os.path.dirname(input_folder), "resized_media"
        )
    ...
    def resize_images(self, image_paths: List[str]) -> List[str]:
        ...
        if not self.resized_folder:
            raise ValueError(
                "Resized folder not initialized. Call ensure_resized_folder first."
            )
    ...
    def resize_and_trim_videos(self, video_paths: List[str]) -> List[str]:
        ...
        for vid_path in video_paths:
            try:
                ...
                # Resize and trim
                clip = clip.resized(new_size=self.target_size).subclipped(
                    0, min(self.duration, clip.duration)
                )
                output_filename = (
                    f"resized_{os.path.splitext(os.path.basename(vid_path))[0]}.mp4"
                )
                output_path = os.path.join(self.resized_folder, output_filename)
